      ...
      <div className="px-4 flex-grow overflow-y-auto">
        <div className="flex w-full items-center mt-[24px]">
          ...
          <div className="ml-auto flex cursor-pointer" onClick={handleOfficeInfo}>
            <div className="mr-[5px] text-neutral-400 text-sm font-normal font-['Pretendard'] leading-[21px]">
              지점 상세보기
            </div>
            <Image
              src={'/nextArrow.svg'}
        ...
        <div className="flex flex-col w-full mt-[24px]">
          <div className="text-black/opacity-20 text-lg font-bold font-['Pretendard']">
          ...
          <div className="flex flex-row items-center">
            ...
            <div className="text-stone-500 text-xs font-normal font-['Pretendard'] mr-[12px] mt-auto">
              {meetingRoom.meetingRoomFloor < 0
                ? `B${Math.abs(meetingRoom.meetingRoomFloor)}`
                : `${meetingRoom.meetingRoomFloor}`}
              층
            </div>
            <Image
              src={'/capacity.svg'}
        ...
        <div className="px-4 py-2">
          <div className="flex-none w-full h-[50px] py-2 flex items-center cursor-pointer">
            <input
              type="text"
              value={eventName}
        ...
        <div className="flex px-4 my-4">
          <div className="text-black/opacity-20 text-base font-bold font-['Pretendard'] my-auto">
            일정
          </div>
        ...
        <div className="px-4 mt-[24px]">
          <div
            ...
            {showSearch ? (
              ...
              <Image
                src={'/reservation/bottomArrow.svg'}
                width={14}
                height={7}
                alt="arrow"
                className=""
                ...
                <div className="mt-4 flex w-full overflow-x-auto">
                  <div className="flex flex-nowrap">
                    {addedMembers.map((member) => (
                      <div
                        key={member.memberId}
                        className="flex items-center mr-2.5 flex-shrink-0">
              ...
              {(inviteableMembers.length > 0 || nonInviteableMembers.length > 0) && (
                <ul className="mt-2">
                  {inviteableMembers.map((member) => (
                    <li key={member.memberId} className="p-2 flex items-center">
                      ...
                      {invitedMemberIds.includes(member.memberId) ? (
                        <Image
                          src="/reservation/InvitedUser.svg"
                          width={28}
                          height={28}
                          alt="invited user"
                          className="ml-auto rounded-full cursor-not-allowed"
                        />
                      ) : (
                        <Image
                          src="/reservation/CanInviteUser.png"
                          width={28}
                          height={28}
                          alt="invite user"
                  ...
                  {nonInviteableMembers.map((member) => (
                    <li
                      key={member.memberId}
                      className="p-2 flex items-center opacity-50">
                      <Image
                        src={member.imageUrl}
                        width={32}
                        height={32}
                      ...
                      <div className="flex flex-col">
                        <div className="flex flex-row items-center">
                          <span>{member.memberName}</span>
                          <Image
                            src={'/reservation/ExclamationMark.svg'}
                            width={14}
                            height={14}
                            alt="ExclamationMark"
                            className="ml-[6.5px]"
                      ...
                      <Image
                        src={'/reservation/CantInviteUser.svg'}
                        width={28}
                        height={28}
                        alt="invite user"
        ...
        <div className="flex px-4 items-start mt-[24px]">
          <Image
            ...
            className="mr-2"
          />
          <p className="text-zinc-400 text-xs font-normal font-['Pretendard'] leading-tight">
            일정이 이미 있는 사용자는 참석 멤버로 등록할 수 없어요! 일정을 조정한 뒤
            추가할 수 있습니다.
          </p>
        </div>
        <footer
          className={`w-full text-center py-[30px] ${showSearch ? 'mb-[100px]' : 'mb-[70px]'} left-0 right-0`}>
          <button
